...
システムはナビゲーション先 URL のホスト名を許可リストと照合し、許可対象かどうかを判定しなければならない (SHALL)。許可リストは次を含む: `signin.aws.amazon.com`、`*.console.aws.amazon.com`、`*.awsapps.com`、`*.amazonaws.com`、`*.aws.amazon.com`、`*.awsstatic.com`、`*.cloudfront.net`、`docs.aws.amazon.com`、`health.aws.amazon.com`。
